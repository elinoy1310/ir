...
# 📘 דוח עבודה – קורס אחזור מידע
תיעוד מלא של שלבי עיבוד הנתונים, ההכנה ובניית הייצוגים למסמכי הפרלמנט הבריטי.

---

## שלב 1 – הורדת קבצי XML
**🎯 מטרה:** לאסוף את כל מסמכי המקור הגולמיים (XML) כדי לבנות עליהם את צינור העיבוד.

**📌 ביצוע:**  
נכתב סקריפט (download_debates.py) המבצע הורדה אוטומטית של כל קובץ XML החל מתאריך מסוים ועד סוף הארכיון. הסקריפט שולף את רשימת הקבצים מהאתר הרשמי, מסנן לפי תבנית שם, ומוריד כל מסמך לתיקייה מסודרת. במידה שקובץ כבר קיים – הוא מדולג. נוספו טיפול בשגיאות, מנגנון retry, והתראת התקדמות לכל ההורדה.

**📈 תוצאה והצדקה:**  
התקבלה ספרייה מלאה של כ־935 קבצי XML תקינים, מוכנים לעיבוד. השיטה האוטומטית מבטיחה אחידות, מונעת טעויות ידניות וחוסכת זמן. כל השגיאות טופלו במהלך ההורדה ולא נמצאו חוסרים.

---

## שלב 2 – ניקוי XML וזיהוי קבצים ריקים
**🎯 מטרה:** להפיק טקסט נקי מתגיות ולהבטיח שרק קבצים בעלי תוכן יעברו לעיבוד.

**📌 ביצוע:**  
קבצי ה־XML עובדו בעזרת `lxml`, תוך שימוש ב־itertext() לחילוץ כל הטקסט המופיע בין תגיות. התוכן נשמר כקובצי `.txt` בתיקייה ייעודית. בנוסף, נבנה סקריפט נוסף שבודק את כל קובצי הטקסט ומזהה האם הם ריקים או כמעט ריקים (פחות מ־10 תווים). קבצים בעייתיים תועדו בלוג, יחד עם שגיאות פרסינג במידת הצורך.

**📈 תוצאה והצדקה:**  
נוצרו קבצים נקיים לחלוטין מתגיות ומוכנים לעבודת טקסט. 13 קבצים ריקים אותרו והופרדו מהעיבוד. שימוש ב־`lxml` הכרחי להתמודדות עם מבנה XML מורכב ובריבוי תתי-אלמנטים. השלב מבטיח איכות נתונים לפני המשך הצינור.

---

## שלב 3 – טוקניזציה (Tokenization)
**🎯 מטרה:** לפרק את הטקסט למילים וסימני פיסוק נפרדים, כך שכל יחידת משמעות תעובד בצורה עקבית.

**📌 ביצוע:**  
סקריפט ה־tokenize_clean_text.py השתמש ב־Regex כדי לזהות מילים בעברית ובאנגלית, כולל תווים מיוחדים כמו גרשיים. סימני פיסוק הופרדו מהטקסט ושמורים כטוקנים עצמאיים. התוצר עבור כל קובץ נשמר בתיקייה ייעודית. הטוקניזציה נשמרה אחידה בין כל המסמכים ומאפשרת בהמשך לבנות ייצוגים מספריים מדויקים.

**📈 תוצאה והצדקה:**  
התקבלו מסמכים מטוקננים בהם כל מילה וכל סימן פיסוק מופיעים כתוקן בודד. השיטה מפחיתה בעיות כמו "government" ו־"government." שנחשבות שונות. התוצאה יציבה ונקייה לקראת lemmatization ולחישובי TF-IDF.

---

## שלב 4.1 – Lemmatization
**🎯 מטרה:** לצמצם וריאציות לשוניות על ידי המרה לצורות בסיס (lemmas), ולהקטין את מספר המאפיינים.

**📌 ביצוע:**  
הטוקנים עובדו באמצעות spaCy (מודל en_core_web_sm). לכל מילה נקבע חלק הדיבר (POS), ולאחר מכן הופקה צורת השורש שלה. כל קובץ נכתב מחדש בתיקייה “lemmatized_text”. תנאי סינון הכוללים הסרת stopwords ומילים נדירות הוחלו על מנת למנוע מילוי מטריצות במאפיינים חסרי משמעות.

**📈 תוצאה והצדקה:**  
מספר המאפיינים הצטמצם מכ־29,000 מילים לכ־23,000 למות. זה מעיד על הפחתת כפילויות ורעש לשוני. המטריצות שנבנו על בסיס למות הפכו קומפקטיות, מדויקות וקלה יותר לעיבוד, מה שמשפר את איכות המודלים בהמשך.

---

## שלב 4.2 – בניית ייצוגים סמנטיים (Word2Vec)
